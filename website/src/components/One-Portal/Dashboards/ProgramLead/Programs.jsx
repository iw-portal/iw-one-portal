...
export default function Programs({ user }) {
  ...
  `,
    ...
    const volunteers = mergedRows
      .filter(
        (row) =>
          row.role === "volunteer" &&
          row.person_id !== selectedProgram?.lead_id &&
          row.person_id !== selectedProgram?.co_lead_id,
      )
      .map((row) => row.people)
      .filter(Boolean);

    const uniqueVolunteers = Array.from(
      new Map(volunteers.map((person) => [person.id, person])).values(),
    );

    setBuddyOptions(uniqueVolunteers);
