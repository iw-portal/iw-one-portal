            ...
            <div className="space-y-3">
              {categories.map((cat) => (
                ...
                >
                  ...
                  <span className="text-gray-700">{formatCategory(cat)}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="md:col-span-3 grid md:grid-cols-1 lg:grid-cols-2 gap-10">
            {filteredCourses.map((course) => (
              ...
              >
                ...
                <img
                  src={
                    course.image_url?.replace("..", "") || "/placeholder.png"
                  }
                  alt={course.course_title}
                  className="rounded-lg mb-5 w-full h-48 object-contain"
                />
          ...
          {isModalOpen && selectedCourse && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-center justify-center z-50">
              <div className="bg-white rounded-xl max-w-3xl w-full p-8 relative overflow-y-auto max-h-[90vh]">
                ...
                <img
                  src={
                    selectedCourse.image_url?.replace("..", "") ||
                    "/placeholder.png"
                  }
                  alt={selectedCourse.course_title}
                  className="rounded-lg mb-5 w-full h-48 object-contain"
                ...
                {selectedCourse.review && selectedCourse.review !== "None" && (
                  <div className="border-l-4 border-[#e0705d] pl-4 italic text-gray-600">
                ...
                <div className="flex flex-wrap gap-2 mt-6">
                  {selectedCourse.category && (
                    <span className="text-xs bg-gray-200 px-2 py-1 rounded">
                      {formatCategory(selectedCourse.category)}
                    </span>
                  )}
                </div>
              </div>
          ...
          <section className="py-10 mb-10 md:col-span-4">
            <div className="w-full mx-auto px-6 flex flex-wrap justify-center gap-7">
              {buttons.map((button) => (
                <button
                  key={button.id}
                  className="bg-[#e16a5b] font-bold text-white px-6 py-4 rounded-md text-sm hover:bg-[#cf5b4c] transition"
                >
                  {button.title}
                </button>
              ))}
